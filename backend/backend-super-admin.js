...
const validarRespuestas = async (cedula, respuestas) => {
    const [filasPreguntas] = await db.query(
        "SELECT `Respuesta` FROM `pregunta_superadmin` WHERE `administrador_Cedula` = ? AND `Status` = 1 ORDER BY `ID`",
        [cedula]
    );
...
const verificarTokenSuper = (req) => {
    try {
        return jwt.verify(req.cookies['superadmin-token'], process.env.JWT_SECRET);
    } catch {
        return null;
    }
};
...
module.exports = (app) => {
    app.post("/superadmin/login", async (req, res) => {
        ...
        try {
            const [filas] = await db.query(
                "SELECT s.Cedula AS cedula, s.Correo AS correo, lc.Clave AS clave FROM superadmin s JOIN lista_clave_superadmin lc ON s.Cedula = lc.medico_Cedula WHERE s.Correo = ? AND lc.Status = 1 AND s.Status = 1",
                [correoNormalizado]
            );
            ...
            const token = jwt.sign(
                { cedula: usuario.cedula, correo: usuario.correo, rol: "SuperAdmin" },
                process.env.JWT_SECRET,
                { expiresIn: "1h" }
            );

            res.cookie("superadmin-token", token, {
                httpOnly: true,
                sameSite: "Strict",
                maxAge: 3600000,
    ...
    app.get("/superadmin/me", async (req, res) => {
        const usuarioSesion = verificarTokenSuper(req);
        if (!usuarioSesion) return res.status(403).json({ error: "No autorizado" });
        try {
            const [filas] = await db.query(
                "SELECT Primer_Nombre AS primerNombre, Segundo_Nombre AS segundoNombre, Primer_Apellido AS primerApellido, Segundo_Apellido AS segundoApellido, Correo AS correo FROM superadmin WHERE Cedula = ? AND Status = 1",
    ...
    app.get("/superadmin/acceso", (req, res) => {
        const usuarioSesion = verificarTokenSuper(req);
        if (!usuarioSesion) return res.status(403).json({ error: "No autorizado" });
        res.set({
        ...
        res.sendFile(path.join(__dirname, "..", "public", "acceso-superadmin.html"));
    });

    app.post("/superadmin/logout", (req, res) => {
        res.clearCookie("superadmin-token");
        res.json({ success: true });
    });

    app.post("/superadmin/recover-lookup", async (req, res) => {
        ...
        try {
            const [filas] = await db.query(
                "SELECT Cedula FROM superadmin WHERE Correo = ? AND Cedula = ? AND Status = 1",
            ...
            const [preguntasRows] = await db.query(
                "SELECT ID as id, Pregunta as pregunta FROM pregunta_superadmin WHERE administrador_Cedula = ? AND Status = 1 ORDER BY ID",
                [cedulaNum]
            );
            ...
            console.error("Error en /superadmin/recover-lookup:", error);
            return res.status(500).json({ error: 'Error al buscar usuario', found: false });
        }
    });

    app.post("/superadmin/verify-answers", async (req, res) => {
        ...
        try {
            ...
            console.error("Error en /superadmin/verify-answers:", error);
            return res.status(500).json({ error: 'Error al verificar las respuestas.' });
        }
    });

    app.post("/superadmin/recover-password", async (req, res) => {
        ...
        try {
            ...
            const [claveActual] = await conexion.query(
                "SELECT `ID`, `Clave` FROM `lista_clave_superadmin` WHERE `medico_Cedula` = ? AND `Status` = 1",
                [cedulaNum]
            );
            ...
            const [clavesPrevias] = await conexion.query(
                "SELECT 1 FROM `lista_clave_superadmin` WHERE `medico_Cedula` = ? AND `Clave` = ? UNION SELECT 1 FROM `historico_clave_superadmin` h JOIN `lista_clave_superadmin` l ON h.`lista_clave_administrador_ID` = l.`ID` WHERE l.`medico_Cedula` = ? AND h.`Clave` = ? LIMIT 1",
            ...
            await conexion.query(
                "UPDATE `lista_clave_superadmin` SET `Status` = 0 WHERE `medico_Cedula` = ? AND `Status` = 1",
                [cedulaNum]
            );

            await conexion.query(
                "INSERT INTO `historico_clave_superadmin` (`Clave`, `Status`, `lista_clave_administrador_ID`) VALUES (?, ?, ?)",
                [claveActual[0].Clave, 0, claveActual[0].ID]
            );

            await conexion.query(
                "INSERT INTO `lista_clave_superadmin` (`medico_Cedula`, `Clave`, `Status`) VALUES (?, ?, ?)",
            ...
            console.error("Error en /superadmin/recover-password:", error);
